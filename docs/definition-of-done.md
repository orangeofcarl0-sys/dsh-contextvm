...
## 补充：真实长对话实验（2026-09-17 续六）

在真实 web 应用里做了一次长对话实验：新建会话 → 第一轮把两个精确事实**不加强调地埋进
一段清单中间**（`验收编号 CAL-7Q2X-88`、`基准波长 1064.37nm`）→ `/contextvm on` → 两轮
无关话题 → 把该会话的压缩阈值调到 6% 逼出宿主的自动压缩 → 用召回查询本地编译该会话。

### 实验挖出一个真缺陷：`lastTurn` 把宿主样板当成"本轮用户查询"

队列里待抽取记录的 `query` 字段是 **`Current runtime context. This snapshot supersedes …`** ——
`lastTurn()` 只按宿主事件类型找最后一条 `user/message`，而宿主**把自己注入的运行时快照
也写成 user/message**。§4.1 的分类此前只用在镜像上，这里漏了。后果：增量抽取被喂宿主样板 →
`delta_unparseable`、单次输出 **2524 token** 的畸形推理（把免费模型的产能白烧近两分钟）、
**状态项始终为 0**。修法与 §4.1 同向（缺 kind 按用户处理，其余排除），并加回归测试。

这类缺陷**只有真实对话才会暴露**：假宿主不会注入运行时快照，单测也不会去构造
"宿主把样板写成 user/message" 的事件序列。

### 实验测到的运行现实

| 现象 | 实测 |
|---|---|
| 宿主自动压缩确实触发（6% 生效） | 但**连续 3 次失败**：`[ac] compact … error: 429 upstream rate-limited` —— 压缩本身要调模型生成摘要，免费模型被限流时它也做不了 |
| 我方增量抽取同一时段 | 同样撞 429，但被正确分类（`delta_provider_error` + 重试），队列保留不丢 |
| 重会话里的抽取开销 | **100–134 秒/次、~2500 output token**（不可见推理），而全新 headless 会话只需 5–20 秒 —— 距 3000 的 soft cap 已经很近，印证了此前把上限从 1500 提到 3000 的必要性 |
| 注入规模 | 本地编译该会话：**189 token**（recent 119 / evidence 41 / 契约 29），两个埋入事实都在注入里 ✓ |
| 开启前的历史 | 第一轮是在休眠状态下发生的，其事件仍在索引里并能被注入 ✓ |

### 长距离召回：另做了一个可复现的受控实验补上（`tests/long-conversation.test.mjs`）

真机那轮只跑到 3 轮（第一轮消息仍在 recent 窗口内），所以"答对"不能归功于远距离检索。
但那件事本身是**确定性**的（取决于我们注入什么，而不是模型的推理），因此另做了一个离线、
可复现的受控实验，把条件压到最紧：

- 第一轮埋入精确事实（`CAL-7Q2X-88` / `1064.37nm`），**不加任何强调**地埋在一段清单中间；
- 再灌 25 轮填充 → 共 **51 个事件**，远超 `recent_events(40)` 的窗口；
- 假 LLM 生成的摘要**刻意不含**该事实 → 摘要兜底路径被排除。

| 用例 | 结果 |
|---|---|
| 状态路径：事实经 delta 落库后 | 从 `Active decisions / facts` 注入 ✓，且**不在** recent 窗口内 |
| **纯检索路径**：完全没有状态 | 仍召回早期原文，作为 `Retrieved evidence` 给出并带 source id ✓ |
| **朴素基线对照**：只给最近 40 条 | **拿不到**该事实（编号与数值都不在）→ 量化了插件的增量 |
| 多轮之后的状态卫生 | 无重复 key、active 项 provenance 100%、队列无积压 ✓ |
...
## 补充：长跑被自己的注入压垮（2026-09-17 续八）

第二轮长跑（P2 世界层）**开场即死**，日志给出了完整链条：

```
context_compiled token_count: 142 → 7955 → 30287 → 40782 → 49030
上游容量拒绝（网关计数 262144 / 上限 270828），注入缩放降至 0.7，第 1 次重试
注入缩放降至 0.49，第 2 次重试
已达收缩重试上限(2)，交还宿主处理   → 宿主也失败 → 长跑终止
```

两件事同时成立：

1. **工具活动修复生效了** —— 注入从 142 涨到 49,030，正是因为 agent 读的大文件现在真的进了
   recent 窗口（此前一条都进不来）；
2. **于是我们成了压垮请求的那部分** —— 网关按 1.693 倍保守估算，262,144 的网关计数 ≈ 15.5 万
   真实 token，我们的 4.9 万占了近三分之一；而**自适应收缩只缩我们这部分**，宿主自身已接近
   上限时缩不动，整轮就死了。

**根因**：我们按 `W` 的**比例**做预算，却不知道**宿主自己已经用了多少**。宿主明明提供了
`ctx.tokenMeter.measure(session).totalTokens`（auto-compact 就用它），我们没用。

**修法（新增主动余量检查）**：装配前量出宿主压力，`余量 = hardInputCap − 宿主压力 − 输出预留`：
- 余量为零 → **整段不注入**并如实上报（`injection_skipped_no_headroom`），把处理权交回宿主的压缩；
- 余量不足 → 按余量收缩（复用既有缩放，**只降不升**，避免与"被拒→重试"抖动）；
- 拿不到 tokenMeter → 行为不变（MUST NOT 因此中断注入）。

顺带否掉一个我曾提议的优化：**每步重复编译不值得缓存**。实测单次编译 2.0 ms（100 事件）
→ 32.6 ms（5,000 事件 ≈ 设计上限量级），100 步累计 0.2–3.3 秒，而同期模型调用是 17–100 分钟
（占 0.1–0.3%）；且按输入指纹的缓存**几乎不会命中**（每步都追加事件）。故不做。
