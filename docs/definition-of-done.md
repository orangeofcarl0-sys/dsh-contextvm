...
3 轮会话里第一轮的消息仍在 recent verbatim 窗口内，所以"埋入事实能被答对"**不能**归功于
远距离检索。真正的长对话区间（事实被挤出 recent 窗口 + 压缩成功）本轮没达到：免费模型的
限流既让压缩失败，也让跑 40–60 轮在时间上不可行。该断言目前由 1M 合成验收
（`acceptance.1m.test.mjs`，Exact identifier recall 100%）覆盖。
