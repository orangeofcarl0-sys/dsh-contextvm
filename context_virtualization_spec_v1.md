...
23. **新增 §13.2.1：工具按会话注册，默认休眠**。v1.3 全局注册全部工具，其 schema
    （实测约 1200–1433 token）随该 profile 下**每个会话**的请求发送，与用户是否用本系统无关 ——
    比注入的上下文（82–257 token）贵一个量级，是最大的一处上下文污染。现改为：默认休眠
    （不注册工具、不注入、不抽取，请求零足迹）；用户在该会话执行 `/contextvm on` 后，
    工具经 `agent.ctx.tools` 注册进**该会话自己的作用域**。拿不到作用域时 MUST 如实失败，
    MUST NOT 全局兜底；输出契约与工具同生共死（工具未注册时不注入调用指令）。
    附带实测：8 个工具的 token 成本里 **77% 是 JSON 结构**（922/1202），描述仅 280、
    `commit_state` 的枚举仅 64 —— 故"压缩描述/枚举"收益极小，真正的杠杆是合并工具（未做）。
...
### 13.2.1 工具 MUST 按会话注册，MUST NOT 全局注册（v1.4 真机实测补充）

v1.3 的实现把全部工具**全局注册**。真机实测的后果：这些工具的 schema 合计约 **1200–1433 token**，
随该 profile 下**每一个会话**的请求发送 —— 与用户是否用本系统无关。这比本系统注入的上下文
（82–257 token）贵一个量级，是当时最大的一处上下文污染（另两处：输出契约诱导的可见工具调用、
两条 delta 路径写重）。

因此新增门控（实现见 `lib/app/session_mode.js` 与 `lib/host/seams.js`）：

- **默认休眠**：不注册任何工具、不注入任何内容、不做轮末抽取 —— 请求层面**零足迹**；
- **显式开启**（用户在该会话执行 `/contextvm on`）：工具经 **`agent.ctx.tools`** 注册进
  **该会话自己的作用域**，注入与轮末抽取同时生效；
- 拿不到 agent 作用域时 MUST 如实失败，**MUST NOT 退化为全局注册**（那会悄悄破坏隔离）；
- **输出契约与工具 MUST 同生共死**：工具未注册时 MUST NOT 注入"如何调用该工具"的指令段
  （否则等于让模型去调用一个不存在的工具）；
- 模式按会话持久化；宿主重启后由首次请求时的懒注册补回。
...
两条缝受会话模式门控（§13.2.1）：`system-prompt/assemble` 在休眠时直接放行（不注入），
`agent/turn-stopping` 在休眠时直接返回（不做抽取与摘要）。
