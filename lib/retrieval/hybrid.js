 ...
 */
import { injectability } from '../core/injectability.js';
...
export class Retriever {
  ...
  constructor(deps) {
    ...
    /** 上一轮检索被排除的非内容候选数（观测用，不参与打分）。 */
    this.lastExcluded = { nonContent: 0 };
  }
  ...
  candidates(sessionId, query, opts = {}) {
    ...
    for (const [eventId, comp] of gathered.pool) {
      if ((gathered.injectability.get(eventId) ?? 'content') !== 'content') nonContent += 1;
    ...
    // 观测（与 semanticState 同性质）：本轮有多少候选因"非内容"被排除。
    // 不做计数的话，"宿主样板又混进来了"这类回归将不可见 —— 本轮审计的教训。
    this.lastExcluded = { nonContent };
  ...
  _gatherCandidates({ sessionId, query, lexicalTopK, exact, filters }) {
    ...
    return {
      ...
      // 只对**候选池**取元数据做可注入性判定：池子上限约 lexicalTopK×2，一次 IN 查询即可。
      // MUST NOT 给上面那条全表查询加 metadata_json —— 1M 会话有 5000+ 事件，
      // 每次查询都解析全部 metadata 是白付的代价。
      injectability: this._injectabilityOf([...pool.keys()]),
  ...
  /**
   * 候选池的可注入性（§7.1）：宿主托管的上下文与我们自己的记账事件**强降权**，
   * 使它们不再抢占榜首 —— 但保留在池中，工具查询在"确实只有它匹配"时仍能召回。
   *
   * @param {string[]} eventIds
   * @returns {Map<string, 'content'|'host_managed'|'bookkeeping'>}
   */
  _injectabilityOf(eventIds) {
    const out = new Map();
    if (eventIds.length === 0) return out;
    const placeholders = eventIds.map(() => '?').join(',');
    const rows = this.db
      .prepare(`SELECT event_id, event_type, metadata_json FROM raw_events WHERE event_id IN (${placeholders})`)
      .all(...eventIds);
    for (const r of rows) out.set(r.event_id, injectability(r.event_type, r.metadata_json));
    return out;
  }
  ...
  _scoreCandidate({ sessionId, eventId, comp, gathered, exact, filters }) {
    ...
    // 非内容事件不进证据（§4.1 / §7.1）：宿主托管的上下文（运行时快照 / 技能目录）与
    // 我们自己的 `state_delta` 记账事件，只在索引里可追溯，MUST NOT 作为证据注入。
    // 真机实测：**仅降权不够** —— 它们仍会以邻居扩展的形式把上万字符拖进证据段，
    // 使证据段占到注入量的 96%（而真实对话只有几百字符），且曾以 0.871 占据榜首。
    if ((gathered.injectability.get(eventId) ?? 'content') !== 'content') return null;
