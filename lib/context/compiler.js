...
/** §8.3：GLOBAL 未做穷举扫描时 MUST 明确告知，MUST NOT 声称"已检查全部"。 */
...
export class Compiler {
  ...
  _collectRecent({ sessionId, caps, notes }) {
    let events = this.raw.recent(sessionId, this.config.recent_events);
    let text = renderRecent(events);
    let tokens = this.tokenizer.estimate(text);
    while (tokens > caps.recent_verbatim && events.length > 1) {
      events = events.slice(1);
      text = renderRecent(events);
  ...
  _buildItems({ stateGroups, evidence, recent, navigator, artifacts, inactive, taskId }) {
    ...
    return [
      { id: 'output_contract', priority: PRIORITY.system_protocol, tokenCount: est(OUTPUT_CONTRACT), section: 'output_contract', content: OUTPUT_CONTRACT },
