...
export class Compiler {
  ...
  _buildItems({ stateGroups, evidence, recent, navigator, artifacts, inactive, taskId, includeOutputContract = true }) {
    ...
    return [
      // 输出契约是"教模型怎么用我们的工具"的指令：工具没注册时 MUST NOT 注入，
      // 否则等于让模型去调用一个不存在的工具（工具按会话注册，见 seams.activateTools）。
