...
export const DELTA_SYSTEM = [
  ...
  '7. 不得把宿主注入的运行时上下文记为状态：环境说明、文件沙箱策略、审批策略、技能目录等',
  '   是宿主每轮自带的环境信息，不是本轮的项目事实。只记录用户与助手在本轮真正谈到的东西。',
  '8. 每条 upsert/open 的 source_event_ids MUST 指向本轮的真实对话事件；指向环境说明会被拒绝。',
  '',
...
export const DELTA_TOOL = Object.freeze({
  ...
  description:
    ...
    '每轮最多提交一次，提交过之后不要再调用；只写变化项；没有变化就不必提交。' +
    '不要用它回答用户问题。',
  parameters: {
    type: 'object',
    properties: {
