...
/**
 * 需要 source 才能标为 active 的类型（§4.2 / §23.2）。
 *
 * 唯一来源：delta.js 也导入这一份，MUST NOT 各写一份 —— 两份列表一旦漂移，
 * 就会出现"某一类在编排层被拦、在存储层放行"的静默缺口。
 *
 * `next_action` 曾不在此列，于是它以 `sourceEventIds: []` 落成 active，
 * 使 §24.3 的"active 项 100% 有来源"在真机上被打破（实测 countWithoutProvenance = 2）。
 */
export const NEEDS_SOURCE_TYPES = new Set([
  'fact',
  'decision',
  'constraint',
  'rejected_option',
...
export class StateStore {
  ...
  upsert(item) {
    ...
    if (status === 'active' && sources.length === 0 && NEEDS_SOURCE_TYPES.has(item.itemType)) {
      throw new Error(
