...
export function mapSessionEvent(event) {
  ...
  switch (event?.type) {
    case 'user/message':
      return { role: 'user', eventType: 'user_message', content: blocksToText(data.content) };
    case 'assistant/message':
      return { role: 'assistant', eventType: 'assistant_message', content: blocksToText(data.message?.content) };
    case 'system/message':
      return { role: 'system', eventType: 'system_note', content: blocksToText(data.content) };
    case 'tool/result':
      return { role: 'user', eventType: 'tool_result', content: blocksToText(data.content ?? data) };
    default:
      return null;
  }
}
...
export function mirrorHostEvent(ctx, vm, session, event) {
  ...
  vm.raw.append({
    ...
    metadata: { hostSeq: event.seq, hostType: event.type },
  });
}
...
async function resolveWindow(seam, session, signal) {
  ...
  const provider = header?.provider ?? seam.vm.config.route.provider;
  const model = header?.model ?? seam.vm.config.route.model;
