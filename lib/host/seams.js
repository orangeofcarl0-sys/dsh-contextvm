 ...
 *
 * 返回的 warnings 只含**本次调用**新增的部分：调用方是在异步拿到 defineTool 之后
 * 才打印的，若让它读 seam.warnings 全量数组，会与挂载期已打印的告警重复。
 */
function registerTools(seam, defineTool) {
  const start = seam.warnings.length;
  const done = () => ({ registered: seam.registered, warnings: seam.warnings.slice(start) });
