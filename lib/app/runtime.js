...
export class Runtime {
  ...
  enqueuePending(entry) {
    ...
    kvSet(this.db, PENDING_KEY, list.slice(-50));
  ...
  async flushPendingDeltas(opts = {}) {
    ...
    for (const [i, entry] of list.entries()) {
      ...
      const res = await this.extractDelta({
      ...
      if (!res.ok) remain.push({ ...entry, attempts: (entry.attempts ?? 0) + 1 });
    }
    kvSet(this.db, PENDING_KEY, remain);
    return { attempted: results.length, remaining: remain.length, results };
  }
}
