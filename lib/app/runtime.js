...
import { applyDelta, recentEventIds, turnSourceEventIds } from '../memory/delta.js';
...
export class Runtime {
   ...
   * 把注入缩放钳制到给定值（**只降不升**）。
   *
   * 与 `shrinkInjection` 的区别：那个是"被上游拒绝后乘一个因子"，这个用于**主动**按
   * 实测余量设定。只降不升是为了避免与"被拒→重试"形成抖动。
   *
   * @param {string} sessionId
   * @param {number} scale
   * @returns {number} 生效后的缩放
   */
  clampInjectionScale(sessionId, scale) {
    const cur = this.injectionScale(sessionId);
    const next = Math.max(0.15, Math.min(cur, scale));
  ...
  /**
   * 编译本次要注入的上下文。
   * @param {{sessionId: string, query?: string, taskId?: string|null, requestedMaxTokens?: number, mode?: string, includeOutputContract?: boolean}} req
   */
  compile(req) {
    ...
    const ctx = this.compiler.compile({
      ...
      includeOutputContract: req.includeOutputContract !== false,
    });
    this.onTelemetry({
  ...
  async extractDelta(req) {
    ...
    if (!parsed.value) {
      // 只报 "unparseable" 是查不动的：真机上有四种完全不同的成因，处置也不同。
      // 逐类上报（finish reason 是宿主给的权威分类，见 §21.7.1）：
      //   error      —— 上游 provider 报错（带 code/status/message），瞬时，重试有意义；
      //   aborted    —— 我们自己的 signal 中止，重试无意义；
      //   max-tokens —— 输出预算被推理耗尽（正文为空），成因是配置，重试无用；
      //   其余       —— 上游偶发空响应或回了非 JSON，瞬时，重试有意义。
      const kind = res.stopReason?.kind ?? null;
      const failure = res.stopReason?.failure ?? null;
      const noRetry =
        kind === 'aborted'
          ? { event: 'delta_aborted', reason: 'aborted', note: '本次辅助调用被中止（signal），不入队' }
          : kind === 'max-tokens' && res.text.length === 0
            ? {
                event: 'delta_budget_exhausted',
                reason: 'budget_exhausted',
                note:
                  '输出上限被推理耗尽、正文为空（目标模型会回传推理，且推理计入输出上限）；' +
                  '请调大 output.state_delta_soft_max_tokens —— 重试无用，故未入队',
              }
            : null;
      if (noRetry) {
        // raw 事件仍在库中可检索（§23.7），丢掉的只是"这一步状态抽取"，且已如实上报
        this.onTelemetry({
          event: noRetry.event,
          session_id: sessionId,
          max_tokens: this.config.output.state_delta_soft_max_tokens,
          note: noRetry.note,
        });
        return { ok: false, reason: noRetry.reason, raw: res.text.slice(0, 500) };
      }
      this.onTelemetry({
        event: kind === 'error' ? 'delta_provider_error' : 'delta_unparseable',
        session_id: sessionId,
        ...
        ...(failure
          ? {
              ...
              failure_message: String(failure.message ?? '').slice(0, 200),
              retry_after_ms: failure.providerRetryAfterMs ?? null,
            }
          : {}),
        sample: res.text.slice(0, 160),
      });
      this.enqueuePending({ sessionId, query, answer, eventIds, error: kind === 'error' ? 'provider_error' : 'unparseable' });
      return { ok: false, reason: kind === 'error' ? 'provider_error' : 'unparseable_delta', raw: res.text.slice(0, 500) };
    }
    // 校验与应用只有一处实现：工具路径与文本 JSON 路径共用 applySubmittedDelta
    return this.applySubmittedDelta({ sessionId, rawDelta: parsed.value, eventIds });
  }

  /**
   * 校验并应用一份 delta（§5.3）。工具路径与文本路径的唯一入口。
   * @param {{sessionId: string, rawDelta: any, now?: string, eventIds?: string[]}} req
   *   `eventIds` 是本轮候选事件（next_action 的来源即取自此，§4.1.1）；省略时按
   *   `turnSourceEventIds` 取默认，使工具路径与文本路径共用同一套来源规则。
  ...
  applySubmittedDelta(req) {
    ...
    const turnEventIds = req.eventIds ?? turnSourceEventIds(this.raw, sessionId);
    const result = applyDelta({ state: this.state, raw: this.raw, sessionId, delta: v.delta, now: req.now, turnEventIds });
