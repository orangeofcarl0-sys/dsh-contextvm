...
export const DEFAULTS = Object.freeze({
  ...
  output: {
    // 目标模型（stealth/union-alpha）**会回传推理**，且推理计入输出上限。
    // 真机实测：max_tokens=500 时 output_tokens 恰好 500、正文 0 字、finish=max-tokens
    // —— 预算被推理吃光，delta 永远抽不出来（表面"正常运行"，实则状态从不积累）。
    // 上限提到 2000 后同一任务两次都成功（用量 316 / 672），且耗时从 17s 降到 4.2s
    // （不再撞上限）。因此辅助调用的上限 MUST 覆盖"推理开销 + 目标正文"。
    state_delta_target_tokens: 300,
    state_delta_soft_max_tokens: 1500,
    state_delta_hard_max_tokens: 2000,
    // worker 同理：300 会被推理吃光，使 GLOBAL 扫描静默返回空 findings
