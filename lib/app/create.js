...
import { openDb, kvGet, kvSet, defaultDbPath } from '../storage/sqlite.js';
...
export function createContextVm(opts) {
  ...
  // dbPath 的语义（§19 / README）：**null 与 undefined 都表示"用默认路径"**。
  //
  // 这里曾写成 `opts.dbPath ?? config.dbPath ?? ':memory:'` —— 而默认配置就是
  // `dbPath: null`，于是 `null ?? ':memory:'` 得到内存库：零配置安装（文档推荐的用法）
  // 实际**什么都不持久化**，重启即丢失全部 authoritative state / episode 摘要 / artifact
  // （这些来自模型 delta，无法从宿主 session log 重建）。真机跑零配置才发现。
  const configuredDbPath = opts.dbPath ?? config.dbPath ?? null;
  const db = openDb(configuredDbPath === null ? defaultDbPath() : configuredDbPath);
