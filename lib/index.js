...
export function apply(ctx, rawConfig = {}) {
  ...
  void (async () => {
    try {
      ...
      // 日志必须反映**实际生效**的模式：配置成 active 时还写"默认休眠"就是撒谎
      logger(
        'info',
        vm.config.sessionMode === 'active'
          ? '工具定义器已就绪：sessionMode=active，每个会话默认开启（工具仍按会话注册）'
          : '工具定义器已就绪：默认休眠，`/contextvm on` 在本会话开启（工具按会话注册）',
      );
    } catch (err) {
