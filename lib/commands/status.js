...
export function renderStatus(vm, invocation) {
  ...
  const active = sessionId ? safe(() => isSessionActive(vm.db, sessionId, vm.config?.sessionMode), '未知') : '未知';
  lines.push(
    active === '未知'
      ? '模式: 未知（取不到会话或读库失败）'
      : active
...
export function registerStatusCommand(ctx, vm, deps = {}) {
  ...
  try {
    const dispose = commands.register({
      ...
      // MUST 声明 input：web 客户端的 matchEnter 规则是 ——
      //   desc.input !== undefined → 认领整行并按命令执行；
      //   否则带参数的行会掉进"默认通道"被当作**普通消息发给模型**（裸命令才直接执行）。
      // 真机实测：`/contextvm on` 因此被当成用户消息送进模型，表现就是"命令输入无反应"。
      // hint 同时是客户端 `leadingClaim` 的输入提示。
      input: { hint: 'on 开启本会话 / off 关闭 / 留空显示状态' },
      handler: (invocation) => {
        try {
