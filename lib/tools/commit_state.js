...
import { jsonSchemaToParameterSpec } from './spec.js';
...
/**
 * 工具参数规格：**由 `DELTA_TOOL.parameters` 派生**，不另写一份。
 *
 * 曾经手写过一份，已经漂移出真 bug：提示词要求空增量写 `next_action: null`，
 * 而手写规格是 `{type:'string'}`，模型照提示词调用就被宿主参数校验拒绝
 * （真机才暴露，见 tools/host-contract-audit.mjs 的守卫）。
 */
export function deltaParametersSpec() {
  return jsonSchemaToParameterSpec(DELTA_TOOL.parameters);
}
